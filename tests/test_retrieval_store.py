...
PAGE_TEXT = (
    "An artificial intelligence search problem has an initial state, actions, "
    "a transition model, a goal test, and a path cost. The state space contains "
    "the possible states, while the search tree records the paths generated by "
    "the search algorithm. A heuristic function estimates the remaining cost "
    "from a state to a goal state, letting informed search prioritize the most "
    "promising frontier nodes."
)

SCOPED_URL = "https://inst.eecs.berkeley.edu/~cs188/textbook/search/summary.html"
...
def test_a_working_run_reports_its_counts_and_succeeds(monkeypatch, tmp_path, capsys):
    code = run_retrieve(monkeypatch, tmp_path, SCOPED_URL)
...
def test_the_run_reports_each_skill_as_well_as_the_whole(monkeypatch, tmp_path, capsys):
    run_retrieve(monkeypatch, tmp_path, SCOPED_URL)
    printed = capsys.readouterr().out
